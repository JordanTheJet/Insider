...
export class HomePage {
  ...
  addPlayer(){
    ...
    this.totalPlayers++
    //ion input ID increment
    let getter = document.getElementById("playerList")
    console.log(getter)
    //get the div
    let label = document.createElement("ion-input")
    //create an ion input box
    
    label.style.backgroundColor = "rgb(255, 0, 0)";
    label.style.textAlign = "center";
    label.style.color = "white";
    label.setAttribute("placeholder","###-###-####")
    
    label.id = `player${this.totalPlayers}`
    console.log(label.id)
    //set the id of the input box
    getter.appendChild(label)
    
    //add the ion input box to the div
    //ID METHOD A
    }else{
      console.log("the maximum players is 8")
      //MAKE AN ALERT LATER
    }
    
  }
  submit(){
    this.secretNumber= Math.floor(Math.random() * this.secretWordArr.length);  
    this.secretWord = this.secretWordArr[this.secretNumber];
    console.log('this is secret number', this.secretNumber);
    console.log('this is secret word', this.secretWord); 
    this.masterNumber= Math.floor((Math.random() * this.totalPlayers)+1);
    console.log(this.masterNumber);
    this.insiderNumber = this.generateInsiderNumber();
    console.log(this.insiderNumber);

    this.getPhoneInput();
    this.generateRoles();
    this.sendMessages()
    // this.sms.send(this.player1Number, 'You are playing Insider');
    //LOOP trough to get values
    //ID METHOD A
  }

  generateInsiderNumber(){
    let num = Math.floor((Math.random() * this.totalPlayers)+1)
    while (num === this.masterNumber){
     num = Math.floor((Math.random() * this.totalPlayers)+1);
     console.log(num)
    } return num;
  }
  ...
  getPhoneInput(){
    let i=1
    let playerHolder: string;
    let playerID: string;
    console.log(this.playerStorage)
    while (i<= this.totalPlayers){
      playerHolder = `getter${i}`
      console.log(playerHolder)
      //getter1
      playerID= `player${i}`
      console.log(playerID)
      this.playerStorage[playerID]= {};
      //player1
      this.playerStorage[playerID].index= i
    
      let divHolder = {}
      divHolder[playerHolder]=<HTMLInputElement>document.getElementById(playerID)
      console.log(divHolder[playerHolder])
      //gets the div with the phonenumber

      this.playerStorage[playerID].number= divHolder[playerHolder].value
      console.log(this.playerStorage[playerID].number)
      console.log(this.playerStorage)
  ...
  generateRoles(){
    Object.keys(this.playerStorage).forEach(function(playerID){ });
    let playerID: string;
    let i =1;
    while(i<=this.totalPlayers){
      playerID= `player${i}`
      if(this.playerStorage[playerID].index===this.masterNumber){
        this.playerStorage[playerID].role="Master"
      }
      else if( this.playerStorage[playerID].index===this.insiderNumber){
        this.playerStorage[playerID].role="Insider"
      }
      else {
        this.playerStorage[playerID].role="Commoner"
      }
      i++;
    }
  }
  sendMessages(){
    ...
    while(i<=this.totalPlayers){
      playerID= `player${i}`
      if(this.playerStorage[playerID].role==="Master"){
        
        this.sms.send(this.playerStorage[playerID].number,`Role: Master. Word: ${this.secretWord}. Announce you are the Master and answer questions Yes or No. If the word is found; identify the Insider to win!`)
        console.log(this.playerStorage[playerID].number,`Role: Master. Word: ${this.secretWord}. Announce you are the Master and answer questions Yes or No. If the word is found; identify the Insider to win!`)
        
      }
      else if( this.playerStorage[playerID].role==="Insider"){
        this.sms.send(this.playerStorage[playerID].number,`Role: Insider. Word: ${this.secretWord}. Help the commons find the word and ask Master yes or no questions. Don't get caught to win!`)    
        console.log(this.playerStorage[playerID].number,`Role: Insider. Word: ${this.secretWord}. Help the commons find the word and ask Master yes or no questions. Don't get caught to win!`)    

      }
      else {
        this.sms.send(this.playerStorage[playerID].number,'Role: Commoner. Ask the Master Yes or No questions to guess the word. Find the word, then help the Master find the Insider to win!')
        console.log(this.playerStorage[playerID].number,'Role: Commoner. Ask the Master Yes or No questions to guess the word. Find the word, then help the Master find the Insider to win!')
